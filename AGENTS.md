...
- builds `target/debug/mise` (only when it does not exist yet, with GitHub tokens unset) and symlinks it to `/usr/local/bin/mise`
...
- runs `mise run build` (with GitHub tokens unset) so `target/` is warm for the `mbx`-wrapped cargo (`[wrappers.cargo]` in `mise.toml`) that agents build with
