...
/// Tasks reachable through `run` task entries (`run = [{ task = "..." }]`),
/// which [`resolve_depends`] does not return: the executor resolves those while
/// the run is already in flight. Anything that has to see every task a run will
/// execute before it starts has to walk them here.
pub(crate) async fn resolve_task_entries(
    config: &Arc<Config>,
    tasks: &[Task],
) -> Result<Vec<Task>> {
    let entry_specs = |task: &Task| -> Vec<String> {
        task.run()
            .iter()
            .flat_map(|entry| match entry {
                crate::task::RunEntry::Script(_) => vec![],
                crate::task::RunEntry::SingleTask { task, .. } => vec![task.clone()],
                crate::task::RunEntry::TaskGroup { tasks } => tasks.clone(),
            })
            .collect()
    };
    if tasks.iter().all(|t| entry_specs(t).is_empty()) {
        return Ok(vec![]);
    }
    let all = config.tasks_with_aliases().await?;
    let mut found: Vec<Task> = vec![];
    let mut seen: HashSet<String> = tasks.iter().map(|t| t.name.clone()).collect();
    let mut queue: Vec<Task> = tasks.to_vec();
    while let Some(task) = queue.pop() {
        for spec in entry_specs(&task) {
            let (name, _) = split_task_spec(&spec);
            // An unresolvable entry is the executor's error to report, with the
            // context it has; skip it rather than failing the run early here.
            let Ok(matches) = all.get_matching(name) else {
                continue;
            };
            for matched in matches {
                if !seen.insert(matched.name.clone()) {
                    continue;
                }
                found.push((*matched).clone());
                queue.push((*matched).clone());
            }
        }
    }
    Ok(found)
