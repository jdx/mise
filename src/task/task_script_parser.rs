...
impl TaskScriptParser {
    ...
    fn check_tera_args_deprecation(
        ...
        // Debug assertion to ensure we remove this functionality after 2025.11.0
        let removal_version = Versioning::new("2025.11.0").unwrap();
        debug_assert!(
            *V <= removal_version,
            "Tera template task arguments (arg/option/flag functions) should have been removed in version 2025.11.0. \
             Please remove this deprecated functionality from task_script_parser.rs"
        );

        // Show deprecation warning for versions >= 2025.5.0
        let deprecation_version = Versioning::new("2025.5.0").unwrap();
        if *V >= deprecation_version {
            deprecated!(
                ...
                "Task '{}' uses deprecated Tera template functions (arg(), option(), flag()) in run scripts. \
                 This will be removed in mise 2025.11.0. The template functions require two-pass parsing which \
