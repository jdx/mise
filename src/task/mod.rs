...
"#,
    ...
    #[test]
    fn test_glob_matching_with_ellipsis() {
        use globset::GlobBuilder;
        
        let test_cases = vec![
            // (pattern, path, should_match)
            ("//**/graph", "//go/api/cmd/graph", true),
            ("//**/graph", "//graph", true),
            ("//**/graph", "//projects/graph", true),
            ("//go/**/graph", "//go/api/cmd/graph", true),
            ("//go/**/graph", "//projects/graph", false),
        ];
        ...
        for (pattern, path, expected) in test_cases {
            ...
            let result = matcher.is_match(path);
            assert_eq!(
                result, expected,
    ...
    #[test]
    fn test_get_matching_with_ellipsis_in_middle() {
        use std::collections::BTreeMap;
        use super::{GetMatchingExt, Task};
        
        let mut tasks = BTreeMap::new();
        tasks.insert(
            "//go/api/cmd/graph:generate".to_string(),
            Task {
                name: "//go/api/cmd/graph:generate".to_string(),
                ..Default::default()
            },
        );
        
        // Test case from the issue: //.../graph:generate should match //go/api/cmd/graph:generate
        let result = tasks.get_matching("//.../graph:generate").unwrap();
        assert_eq!(result.len(), 1, "Pattern //.../graph:generate should match //go/api/cmd/graph:generate");
        
        // Also test that //go/...:generate works
        let result = tasks.get_matching("//go/...:generate").unwrap();
        assert_eq!(result.len(), 1, "Pattern //go/...:generate should match //go/api/cmd/graph:generate");
    }
}
