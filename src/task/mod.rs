...
impl Task {
    ...
    pub async fn all_depends(&self) -> Result<Vec<Task>> {
        let config = Config::get().await;
        ...
        for dep in depends.clone() {
            let mut extra = Box::pin(dep.all_depends()).await?;
    ...
    pub async fn parse_usage_spec(
        ...
        let (mut spec, scripts) = if let Some(file) = &self.file {
            ...
            let (scripts, spec) =
                TaskScriptParser::new(cwd).parse_run_scripts(self, self.run(), env).await?;
            (spec, scripts)
        };
