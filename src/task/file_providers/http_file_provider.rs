...
impl TaskFileProvider for HttpTaskFileProvider {
    ...
    fn get_local_path(&self, tmpdir: &PathBuf, file: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_http_task_file_provider_get_local_path() {
       let paths = vec![
        ...
        for path in paths {
            let mocked_server = server.mock("GET", path)
                .with_status(200)
            ...
            let path = provider.get_local_path(&tmpdir.path().to_path_buf(), &mock).unwrap();
            assert!(path.exists());
            assert!(path.is_file());
