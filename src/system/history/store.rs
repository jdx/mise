...
pub(crate) struct ExpandedRule {
    ...
    pub negated: bool,
    /// The portable root `pattern` is relative to — `home` or `config`,
    /// the same roots the tree stores paths under — when the rule names
    /// a place every machine has. Absent when it does not, in which case
    /// `pattern` is the absolute path this machine expanded it to.
    ///
    /// Portability is what decides. A `~` or `$HOME` pattern means the
    /// same thing on every machine, so freezing it to the capturing
    /// machine's home would stop it matching anywhere else — and a rule
    /// that stops matching makes a rollback treat excluded files as
    /// absent and delete them. Any other variable means whatever it
    /// meant there and then, so its expansion is frozen.
    #[serde(default, skip_serializing_if = "Option::is_none")]
