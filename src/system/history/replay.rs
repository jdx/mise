...
fn current_object(repo: &HistoryRepo, path: &Path) -> Result<Option<(String, String)>> {
    ...
    if meta.file_type().is_symlink() {
        ...
        let oid = repo.transient_blob_id(target.to_string_lossy().as_bytes())?;
    ...
    let oid = repo.transient_blob_id(&std::fs::read(path)?)?;
    ...
    for target in &exec.targets {
        ...
        for path in &target.paths {
            ...
            for file in files {
                ...
                let saved_path = repository_path(repo, snapshot, &abs)?;
                let saved = repo.restored_object_at(snapshot, &saved_path)?;
                let mut current = repo.restored_object_at(live, &file)?;
...
fn occupied_restore_dirs(
    ...
    for dir in &exec.restore_dirs {
        ...
        let tree_path = repository_path(repo, live, dir)?;
        let action = match repo.restored_object_at(live, &tree_path)? {
...
pub(crate) fn classify_coverage(coverage: &super::store::Coverage, display: &str) -> PathState {
    ...
    let has_patterns = !coverage.exclude.is_empty()
        || owner.exclude.as_ref().is_some_and(|patterns| !patterns.is_empty())
        || owner.include.as_ref().is_some_and(|patterns| !patterns.is_empty());
...
mod permission_tests {
    ...
    #[test]
    fn directory_permission_recovery_rejects_replaced_paths() -> Result<()> {
        ...
        let step = Step {
            ...
            to: String::new(),
            bits: None,
            dir_bits: vec![(path.clone(), 0o700)],
        };
        assert!(restore_dir_modes(&step, &step.dir_bits).is_err());
        std::fs::create_dir(&path)?;
        restore_dir_modes(&step, &step.dir_bits)?;
        assert_eq!(
