...
impl DotfilesTrack {
    ...
    pub(crate) async fn run(self) -> Result<()> {
        ...
        for target_raw in &self.targets {
            ...
            // re-tracking previews under the entry's own exclude and
            // include lists
        ...
        for target in resolved {
            ...
            if self.dry_run {
                ...
                for glob in &set.entries[entry_index].include {
    ...
    fn entry(
        ...
        if existing.is_some() && written("include") {
            let list = previous_include.unwrap_or_else(|| {
                let mut list = Array::new();
                for pattern in existing.iter().flat_map(|req| &req.include) {
                    list.push(string(pattern.as_str()));
                }
                list
            });
