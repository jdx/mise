...
impl TaskDocs {
    pub async fn run(self) -> eyre::Result<()> {
        ...
        let tasks = config::load_tasks_in_dir(dir, &config.config_files).await?;
        let mut out = vec![];
        for task in tasks.iter().filter(|t| !t.hide) {
            out.push(task.render_markdown(ts, dir).await?);
        }
            ...
            if self.multi {
                if output.is_dir() {
                    for (i, task) in tasks.iter().filter(|t| !t.hide).enumerate() {
                        ...
                        file::write(&path, &task.render_markdown(ts, dir).await?)?;
                    }
                } else {
