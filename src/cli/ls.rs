...
impl Ls {
    ...
    async fn display_user<'a>(&'a self, config: &Config, runtimes: Vec<RuntimeRow<'a>>) -> Result<()> {
        let mut rows = vec![];
        for (ls, p, tv, source) in runtimes {
            rows.push(Row {
                tool: p.clone(),
    ...
    async fn get_prunable_runtime_list(&self) -> Result<Vec<RuntimeRow>> {
        ...
        Ok(prune::prunable_tools(installed_tool.iter().collect()).await?
            .into_iter()
            .map(|(p, tv)| (self, p, tv, ToolSource::Unknown))
            .collect())
    }
    async fn get_runtime_list(&self, config: &Config) -> Result<Vec<RuntimeRow>> {
        ...
        let rvs: Vec<RuntimeRow> = ts
            ...
            .filter(|(_ls, p, tv, source)| !source.is_unknown() || p.is_version_installed(config, tv, true))
...
async fn version_status_from(config: &Config, (ls, p, tv, source): (&Ls, &dyn Backend, &ToolVersion, &ToolSource)) -> VersionStatus {
