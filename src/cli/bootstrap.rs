    ...
    async fn run_from(&self) -> Result<()> {
            ...
            .await?
        {
            ...
            if outcome.setup_held {
                bail!(
                    "the setup from {url} is paused; nothing was bootstrapped. `mise dot status` lists the paths that need attention. Existing files that differ are kept until you decide: `mise dot pull --take-remote-all` takes the repository's version of every conflicting file, or `mise dot pull --take-remote <path>` and `mise dot pull --keep-local <path>` decide one at a time. A path held for another reason, such as a directory where the repository has a file, says so in `mise dot status` and needs that fix instead. Then run `mise bootstrap`"
                );
            }
            let config_dir = system::history::tracked::global_config_dir();
