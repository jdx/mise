...
impl Upgrade {
    pub async fn run(self) -> Result<()> {
        ...
        let ts = ToolsetBuilder::new().with_args(&self.tool).build(&config).await?;
        let mut outdated = ts.list_outdated_versions(self.bump).await;
        if self.interactive && !outdated.is_empty() {
            outdated = self.get_interactive_tool_set(&outdated)?;
        } else if !self.tool.is_empty() {
            outdated.retain(|o| self.tool.iter().any(|t| t.ba.as_ref() == o.tool_version.ba()));
        }
    ...
    async fn upgrade(&self, config: &Config, outdated: Vec<OutdatedInfo>) -> Result<()> {
        let mpr = MultiProgressReport::get();
        let mut ts = ToolsetBuilder::new().with_args(&self.tool).build(config).await?;
        ...
        for outdated_info in &outdated {
            ...
            match ts.install_all_versions(vec![tool_request], &opts).await {
                Ok(versions) => {
    ...
    async fn uninstall_old_version(&self, tv: &ToolVersion, pr: &Box<dyn SingleReport>) -> Result<()> {
        tv.backend()?
