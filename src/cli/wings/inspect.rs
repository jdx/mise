...
impl InspectTarget {
    fn new(reference: &str, digest: Option<&str>, require_digest: bool) -> Result<Self> {
        ...
        let expected_registry = format!("registry.{}", crate::wings::host());
            ...
            "wings inspect only sends credentials to {}; got {}",
            expected_registry,
            parsed.registry
        );
        let digest = digest
...
mod tests {
    ...
    #[test]
    fn inspect_target_rejects_non_wings_registry() {
        let digest = format!("sha256:{}", "a".repeat(64));
        let err = InspectTarget::new(&format!("evil.example.com/acme/node@{digest}"), None, true)
            .unwrap_err();

        assert!(err.to_string().contains("only sends credentials"));
        assert!(err.to_string().contains("registry.mise-wings.en.dev"));
    }
}
