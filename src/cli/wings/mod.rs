...
//!
//! With `wings.enabled = true` AND credentials present, normal
//! tool installs ask the mise-wings catalog resolver for an
//! approved OCI artifact before calling the backend installer.
//! Tools use Wings by default once that global gate is active;
//! set `wings = false` on a specific `[tools]` entry to force
//! that tool through its normal backend installer. No behavior
//! change without both halves of the gate, so an `mise wings
//! login` that doesn't also flip `wings.enabled` is a no-op
//! until the user opts in (typically in `mise.toml`).
