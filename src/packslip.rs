...
/// off, and each leaves a declared skill with nothing on disk behind it.
fn why_missing(group: &[&Resource]) -> String {
    let settings = Settings::get();
    if !settings.skills.fetch {
        "skills.fetch is off, so it was not fetched at install time".to_string()
            ...
            .all(|r| matches!(r.source(), Some(ResourceSource::Exec)))
    {
        ...
        "the install does not hold it; reinstall the tool to fetch it again".to_string()
    }
}
