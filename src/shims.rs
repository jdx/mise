...
use crate::config::{Config, Settings, SettingsExt, load_command_wrappers};
...
pub async fn err_shim_not_found(bin_name: &str) -> color_eyre::Report {
    ...
    let build = async {
        ...
        let ts = crate::toolset::ToolsetBuilder::new().build(&config).await?;
...
///
/// The recovery is the one `windows_launcher_body` in `cli/generate` documents in full: what
