...
static SINGLE_LOCKFILE_CACHE: Lazy<Mutex<HashMap<Vec<PathBuf>, Arc<Lockfile>>>> =
    Lazy::new(Default::default);
static LEGACY_LOCKFILE_PATHS_CACHE: Lazy<Mutex<HashMap<Vec<PathBuf>, IndexSet<PathBuf>>>> =
    Lazy::new(Default::default);
...
fn monorepo_legacy_lockfile_paths(config: &Config) -> IndexSet<PathBuf> {
    let cache_key = config.config_files.keys().cloned().collect();
...
mod tests {
    ...
    fn test_legacy_lockfile_path_discovery_is_cached() {
        ...
        let cache_key = vec![temp_dir.path().join("mise.toml")];
