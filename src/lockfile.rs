...
> = Lazy::new(Default::default);
/// What [`read_all_lockfiles`] derives once per [`Config`]: the paths that key
/// the shared cache, plus the monorepo/legacy discovery results the cache-miss
/// path needs, so a miss doesn't repeat that discovery.
pub(crate) struct LockfileDiscovery {
    cache_key: Vec<PathBuf>,
    monorepo_root: Option<PathBuf>,
    legacy_lockfiles: IndexSet<PathBuf>,
}
...
fn monorepo_legacy_lockfile_paths(config: &Config) -> IndexSet<PathBuf> {
    let Some(cache_key) = config.monorepo_lockfile_discovery_key() else {
        return IndexSet::new();
    };
    cached_legacy_lockfile_paths(cache_key, || {
        discover_monorepo_legacy_lockfile_paths(config)
    })
}

fn cached_legacy_lockfile_paths(
    cache_key: LegacyLockfilePathsCacheKey,
    discover: impl FnOnce() -> IndexSet<PathBuf>,
) -> IndexSet<PathBuf> {
    let mut cache = LEGACY_LOCKFILE_PATHS_CACHE
        .lock()
        .unwrap_or_else(|err| err.into_inner());
    if let Some(paths) = cache.get(&cache_key) {
        return paths.clone();
    }
    let paths = discover();
...
fn discover_monorepo_legacy_lockfile_paths(config: &Config) -> IndexSet<PathBuf> {
    let Some(monorepo_root) = config.monorepo_lockfile_root() else {
        return IndexSet::new();
    };
...
fn read_all_lockfiles(config: &Config) -> Arc<Lockfile> {
    ...
    if let Some(cached) = cache.get(&discovery.cache_key) {
        return Arc::clone(cached);
    }

    // Only the miss path needs these; the hit path above is what runs hundreds
    // of times per invocation.
    let monorepo_root = discovery.monorepo_root.clone();
    let legacy_lockfiles = &discovery.legacy_lockfiles;
...
pub(crate) fn get_locked_version(
    ...
    if let ToolSource::ToolStub(path) = source
        && let Some(tool) = crate::cli::tool_stub::locked_tool_from_stub(
            path,
            specifier,
            &backend.map_or_else(|| Platform::current().to_key(), |b| b.get_platform_key()),
            request_options,
        )?
    {
        return Ok(Some(tool));
...
"#,
    ...
    #[test]
    fn test_legacy_lockfile_path_discovery_is_cached() {
        ...
        let expected = IndexSet::from([temp_dir.path().join("packages/a/mise.lock")]);

        for _ in 0..3 {
            let paths = cached_legacy_lockfile_paths(cache_key.clone(), || {
    ...
    #[test]
    fn test_legacy_lockfile_path_discovery_uses_effective_monorepo_config() {
        let temp_dir = tempfile::tempdir().unwrap();
        let config_path = temp_dir.path().join("mise.toml");
        let discoveries = AtomicUsize::new(0);

        for roots in [["packages/*"], ["crates/*"]] {
            cached_legacy_lockfile_paths(
                (
                    config_path.clone(),
                    Some(true),
