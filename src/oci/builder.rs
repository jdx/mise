...
fn synthesize_embedded_config_toml(
    ...
    render_config_toml(&group_versions(versions.iter().map(|(_, tv)| tv)))
}

/// Groups tool versions by backend short name. Preserves both the order in
/// which tools were first seen and the order of versions within a tool —
/// mise resolves PATH precedence by array position, so this is not cosmetic.
fn group_versions<'a>(
    versions: impl IntoIterator<Item = &'a ToolVersion>,
) -> IndexMap<String, Vec<String>> {
    let mut grouped: IndexMap<String, Vec<String>> = IndexMap::new();
    for tv in versions {
        let entry = grouped.entry(tv.ba().short.clone()).or_default();
        if !entry.iter().any(|v| v == &tv.version) {
            entry.push(tv.version.clone());
        }
    }
    grouped
}

/// Renders grouped tool versions into a `[tools]` document. A single version
/// becomes a string, multiple versions become an array — matching what mise
/// accepts in a source `mise.toml`. Emitting a bare `short = version` per tool
/// would produce duplicate keys for tools pinned to several versions.
fn render_config_toml(grouped: &IndexMap<String, Vec<String>>) -> String {
    let mut s = String::from("# Auto-generated by `mise oci build`. Do not edit.\n[tools]\n");
    for (short, versions) in grouped {
        // One key at a time through the TOML library: shorts and versions can
        // contain `"` and `\` (rare, but possible for e.g. ref/branch
        // specifiers), so serialize rather than interpolate. A single table
        // holding all keys would be a BTreeMap and sort the tools.
        let value = match versions.as_slice() {
            [single] => toml::Value::String(single.clone()),
            many => toml::Value::Array(many.iter().cloned().map(toml::Value::String).collect()),
        };
        let mut tbl = toml::value::Table::new();
        tbl.insert(short.clone(), value);
        s.push_str(&toml::to_string(&tbl).unwrap_or_default());
    }
    s
}
...
mod tests {
    ...
    fn parse_tools(output: &str) -> toml::Value {
        toml::from_str::<toml::Value>(output).expect("output must be valid TOML")["tools"].clone()
    }

    #[test]
    fn single_version_renders_as_string() {
        let mut grouped = IndexMap::new();
        grouped.insert("node".to_string(), vec!["20".to_string()]);

        let tools = parse_tools(&render_config_toml(&grouped));
        assert_eq!(tools["node"].as_str(), Some("20"));
    }
    ...
    fn multiple_versions_render_as_array_in_order() {
        ...
        grouped.insert(
            "java".to_string(),
            vec!["liberica-25".to_string(), "liberica-17".to_string()],
        );

        let tools = parse_tools(&render_config_toml(&grouped));
        let java: Vec<_> = tools["java"]
            .as_array()
            .expect("java must be an array")
            .iter()
            .map(|v| v.as_str().expect("array entries must be strings"))
            .collect();
        assert_eq!(java, ["liberica-25", "liberica-17"]);
    }
}
