...
pub async fn list_releases(repo: &str) -> Result<Vec<GitlabRelease>> {
    ...
    Ok(cache
        .get_or_try_init_async(async || list_releases_(API_URL, repo).await).await?
        .to_vec())
}
...
async fn list_releases_(api_url: &str, repo: &str) -> Result<Vec<GitlabRelease>> {
    ...
    let (mut releases, mut headers) = crate::http::HTTP_FETCH
        .json_headers_with_headers::<Vec<GitlabRelease>, _>(url, &headers).await?;

    if *env::MISE_LIST_ALL_VERSIONS {
        while let Some(next) = next_page(&headers) {
            let (more, h) = crate::http::HTTP_FETCH
                .json_headers_with_headers::<Vec<GitlabRelease>, _>(next, &headers).await?;
...
pub async fn list_tags(repo: &str) -> Result<Vec<String>> {
    ...
    let cache = cache.get(&key).unwrap();
    Ok(cache
        .get_or_try_init_async(async || list_tags_(API_URL, repo).await).await?
        .to_vec())
}
...
async fn list_tags_(api_url: &str, repo: &str) -> Result<Vec<String>> {
    ...
    let (mut tags, mut headers) =
        crate::http::HTTP_FETCH.json_headers_with_headers::<Vec<GitlabTag>, _>(url, &headers).await?;

    if *env::MISE_LIST_ALL_VERSIONS {
        while let Some(next) = next_page(&headers) {
            let (more, h) = crate::http::HTTP_FETCH
                .json_headers_with_headers::<Vec<GitlabTag>, _>(next, &headers).await?;
            tags.extend(more);
            headers = h;
        }
    }
...
pub async fn get_release(repo: &str, tag: &str) -> Result<GitlabRelease> {
    ...
    let cache = cache.get(&key).unwrap();
    Ok(cache
        .get_or_try_init_async(async || get_release_(API_URL, repo, tag).await).await?
        .clone())
}
...
async fn get_release_(api_url: &str, repo: &str, tag: &str) -> Result<GitlabRelease> {
    ...
    let headers = get_headers(&url);
    crate::http::HTTP_FETCH.json_with_headers(url, &headers).await
}
