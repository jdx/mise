...
impl PythonPlugin {
    ...
    async fn install_precompiled(&self, ctx: &InstallContext, tv: &ToolVersion) -> eyre::Result<()> {
...
impl Backend for PythonPlugin {
    ...
    async fn install_version_(
        ...
        if let Some(default_file) = &SETTINGS.python.default_packages_file {
            ...
            if let Err(err) = self.install_default_packages(&config, &default_file, &tv, &ctx.pr).await {
