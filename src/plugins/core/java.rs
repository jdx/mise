...
impl JavaPlugin {
    ...
    async fn fetch_java_metadata(&self, release_type: &str) -> Result<&HashMap<String, JavaMetadata>> {
        ...
        cache.get_or_try_init_async(async || {
    ...
    async fn tv_to_metadata(&self, tv: &ToolVersion) -> Result<&JavaMetadata> {
        ...
        let m = self
            .fetch_java_metadata(&release_type).await?
            .get(&v)
