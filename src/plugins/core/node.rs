...
impl NodePlugin {
    ...
    async fn get_checksum(&self, ctx: &InstallContext, tarball: &Path, version: &str) -> Result<String> {
        ...
        HTTP.download_file(self.shasums_url(version)?, &shasums_file, Some(&ctx.pr)).await?;
    ...
    async fn verify_with_gpg(&self, ctx: &InstallContext, shasums_file: &Path, v: &str) -> Result<()> {
...
impl Backend for NodePlugin {
    ...
    async fn _list_remote_versions(&self) -> Result<Vec<String>> {
        ...
        let versions = HTTP_FETCH
            .json::<Vec<NodeVersion>, _>(base.join("index.json")?).await?
            .into_iter()
            .filter(|v| {
