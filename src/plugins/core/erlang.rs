...
impl ErlangPlugin {
    ...
    async fn install_precompiled(
        ...
        HTTP.download_file(&asset.browser_download_url, &tarball_path, Some(&ctx.pr)).await?;
    ...
    async fn install_precompiled(
        ...
        HTTP.download_file(&asset.browser_download_url, &zip_path, Some(&ctx.pr)).await?;
    ...
    async fn install_via_kerl(&self, _ctx: &InstallContext, tv: ToolVersion) -> Result<ToolVersion> {
...
impl Backend for ErlangPlugin {
    ...
    async fn _list_remote_versions(&self) -> Result<Vec<String>> {
        let versions = if SETTINGS.erlang.compile == Some(false) {
            github::list_releases("erlef/otp_builds").await?
                .into_iter()
