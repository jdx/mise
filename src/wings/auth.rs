...
async fn usable_access_token(creds: credentials::Credentials) -> Result<Option<String>> {
    ...
    match maybe_refresh(&creds).await {
        ...
        Err(e) => {
            if !creds.should_refresh(0)
                && credentials::cached().is_some_and(|current| {
                    current.host == creds.host && current.access_token == creds.access_token
                })
            {
                log::warn!(
                    "wings: auto-refresh failed; using existing access token until it expires. Error: {e:#}"
                );
                return Ok(Some(creds.access_token));
            }
            log::warn!(
                "wings: auto-refresh failed and no valid existing token remains; skipping wings. Error: {e:#}"
            );
            Ok(None)
        }
    }
