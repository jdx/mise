...
pub struct Credentials {
    ...
    /// `Authorization: Bearer <token>` to the wings catalog
    /// and registry.
    ...
    pub org: String,
    /// Enrolled device id for device-bound dev auth. Legacy
    /// paste-token credentials may not have one.
    #[serde(default)]
    pub device_id: Option<String>,
}

pub struct DevAuthCredentials {
    pub host: String,
    pub access_token: String,
    pub expires_in: u64,
    pub refresh_token: String,
...
impl Credentials {
    ...
    pub fn from_dev_auth(input: DevAuthCredentials) -> Result<Self> {
        let now = now_unix();
        let access_exp = parse_jwt_exp(&input.access_token).unwrap_or_else(|| {
        ...
        Ok(Self {
            ...
            refresh_expires_at: now + input.refresh_expires_in as i64,
            host: input.host,
