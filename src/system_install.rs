...
/// Runs before configuration, logging, migrations, or the async runtime.
pub(crate) fn early_main() -> Option<std::process::ExitCode> {
    if std::env::args_os().nth(1).as_deref() == Some(std::ffi::OsStr::new(HELPER)) {
        ...
        return Some(match result {
            ...
            Err(err) => {
                eprintln!("mise ERROR {err:#}");
                std::process::ExitCode::FAILURE
            }
        });
    }
    if reject_sudo(sudo::is_root(), std::env::var_os("SUDO_UID").is_some()) {
        eprintln!(
            "mise ERROR do not run mise with sudo; run mise as your user. System installations invoke sudo when publishing files."
        );
...
fn reject_sudo(root: bool, sudo_uid: bool) -> bool {
    root && sudo_uid
}
...
fn publish(input: impl Read) -> Result<()> {
    ...
    sudo::run_with_reader(&executable.to_string_lossy(), &[HELPER.into()], input)
}
...
mod tests {
    ...
    #[test]
    fn sudo_guard_and_archive_boundary() -> Result<()> {
        assert!(reject_sudo(true, true));
        assert!(!reject_sudo(true, false)); // containers/direct root
        assert!(!reject_sudo(false, true)); // unprivileged children
