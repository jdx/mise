...
impl ToolVersion {
    ...
    pub async fn resolve(request: ToolRequest, opts: &ResolveOptions) -> Result<Self> {
        ...
        let tv = match request.clone() {
            ToolRequest::Version { version: v, .. } => Self::resolve_version(request, &v, opts).await?,
            ToolRequest::Prefix { prefix, .. } => Self::resolve_prefix(request, &prefix, opts).await?,
            ToolRequest::Sub {
                sub, orig_version, ..
            } => Self::resolve_sub(request, &sub, &orig_version, opts).await?,
            _ => {
    ...
    pub async fn latest_version(&self) -> Result<String> {
        ...
        let tv = self.request.resolve(&opts).await?;
    ...
    async fn resolve_version(
        ...
        match v.split_once(':') {
            ...
            Some((part, v)) if part.starts_with("sub-") => {
                ...
                return Self::resolve_sub(request, sub, v, opts).await;
            }
            _ => (),
        }
    ...
    async fn resolve_sub(
        ...
        let v = match v {
            ...
            _ => Config::get().await.resolve_alias(&backend, v).await?,
        };
        let v = tool_request::version_sub(&v, sub);
        Box::pin(Self::resolve_version(request, &v, opts)).await
    }
