...
/// so every operation that mutates either the marker or an install path uses
/// this logical identity. The marker path is only the lock identity; the lock
/// itself remains a separate stable file under the lockfiles cache.
