...
impl Toolset {
    ...
    pub async fn resolve(&mut self) -> eyre::Result<()> {
        ...
        for (_ba, tvl) in self.versions.iter_mut() {
            // TODO: parallelize this
            tvl.resolve(&Default::default()).await?;
        }
        Ok(())
    }
    pub async fn install_missing_versions(
        ...
        let versions = self
            ...
            .map(|tv| tv.request)
            .collect_vec();
        let versions = self.install_all_versions(versions, opts).await?;
    ...
    pub async fn install_all_versions(
        ...
        while !leaf_deps.is_empty() {
            ...
            installed.extend(self.install_some_versions(leaf_deps, opts).await?);
    ...
    async fn install_some_versions(
        ...
        for (ba, trs) in queue {
            ...
            tset.spawn(async move {
                ...
                let mut installed = vec![];
                for tr in trs {
                    let tv = tr.resolve(&opts.resolve_options).await?;
    ...
    pub async fn list_installed_versions(&self) -> Result<Vec<(Arc<dyn Backend>, ToolVersion)>> {
        ...
        let mut jset: JoinSet<Result<Vec<(Arc<dyn Backend>, ToolVersion)>, eyre::Report>> =
            JoinSet::new();
        ...
        while let Some(res) = jset.join_next().await {
            match res {
                Ok(Ok(v)) => versions.extend(v),
                Ok(Err(e)) => {
                    warn!("Error listing installed versions: {e:#}");
                }
                Err(e) => {
                    warn!("Error listing installed versions: {e:#}");
                }
    ...
    pub async fn list_outdated_versions(&self, bump: bool) -> Vec<OutdatedInfo> {
        ...
        for (t, tv) in self.list_current_versions() {
            jset.spawn(async move {
                ...
                OutdatedInfo::resolve(tv.clone(), bump)
                    .await
                    .unwrap_or_else(|e| {
        ...
        while let Some(res) = jset.join_next().await {
            match res {
                ...
                Ok(None) => {}
                Err(e) => {
                    warn!("Error getting outdated info: {e:#}");
                }
            }
        }
        outdated
    ...
    pub async fn list_paths(&self) -> Vec<PathBuf> {
        ...
        for (p, tv) in self.list_current_installed_versions(&config) {
            jset.spawn(async move {
                p.list_bin_paths(&tv).await.unwrap_or_else(|e| {
                    warn!("Error listing bin paths for {tv}: {e:#}");
                    Vec::new()
                })
            });
        ...
        while let Some(res) = jset.join_next().await {
            match res {
                Ok(paths) => out.extend(paths.into_iter().filter(|p| p.parent().is_some())),
                Err(e) => {
                    warn!("Error listing paths: {e:#}");
                }
            }
        }
        out
    ...
    pub async fn which_bin(&self, bin_name: &str) -> Option<PathBuf> {
        let Some((p, tv)) = Box::pin(self.which(bin_name)).await else {
            return None;
        };
    ...
    pub async fn install_missing_bin(
        ...
        for plugin in plugins {
            ...
            if !versions.is_empty() {
                let versions = self
                    .install_all_versions(versions.clone(), &InstallOptions::default())
                    .await?;
                if !versions.is_empty() {
                    config::rebuild_shims_and_runtime_symlinks(&versions).await?;
                }
