use crate::{aqua::aqua_registry::{
    AquaChecksumType, AquaMinisignType, AquaPackage, AquaPackageType, AQUA_REGISTRY
}};
...
impl Backend for AquaBackend {
    ...
    async fn install_version_(&self, ctx: &InstallContext, mut tv: ToolVersion) -> Result<ToolVersion> {
        ...
        let url = match self.fetch_url(&pkg, &v).await {
            ...
            Err(err) => {
                ...
                self.fetch_url(&pkg, &v).await.map_err(|e| err.wrap_err(e))?
            }
        };
    ...
    async fn list_bin_paths(&self, tv: &ToolVersion) -> Result<Vec<PathBuf>> {
        let pkg = AQUA_REGISTRY.package_with_version(&self.id, &tv.version).await?;
        ...
        if !tv.checksums.contains_key(filename) {
            if let Some(checksum) = &pkg.checksum {
                if checksum.enabled() {
                    ...
                    self.cosign_checksums(ctx, pkg, v, tv, &checksum_path).await?;
        ...
        if let Some(minisign) = &pkg.minisign {
            ...
            let sig_path = match minisign._type() {
                AquaMinisignType::GithubRelease => {
                    ...
                    let url = github::get_release(&format!("{repo_owner}/{repo_name}"), v).await?
                        .assets
                        .into_iter()
                        .find(|a| a.name == asset)
            ...
            if let Some(slsa_bin) = self.dependency_which("slsa-verifier").await {
                ...
                let provenance_path = match slsa.r#type.as_deref().unwrap_or_default() {
                    "github_release" => {
                        ...
                        let url = github::get_release(&repo, v).await?
                            .assets
                            .into_iter()
                            .find(|a| a.name == asset)
                            .map(|a| a.browser_download_url);
...
async fn get_versions(pkg: &AquaPackage) -> Result<Vec<String>> {
    ...
    let mut versions = github::list_releases(&format!("{}/{}", pkg.repo_owner, pkg.repo_name)).await?
        .into_iter()
        .map(|r| r.tag_name)
        .collect_vec();
