...
impl<'a> AquaOptions<'a> {
    ...
    fn canonical_var_options(&self) -> BTreeMap<String, &toml::Value> {
        ...
        for (key, value) in self.values.raw().iter() {
            ...
            if key == "vars" {
                if let toml::Value::Table(table) = value {
                    Self::insert_nested_var_options(&mut ranked, table);
                }
                continue;
            }

            let (key, priority) = if let Some(key) = key.strip_prefix("vars.") {
                (key.to_string(), AquaVarOptionPriority::Prefixed)
            } else {
                (key.clone(), AquaVarOptionPriority::Plain)
            };
            Self::insert_var_option(&mut ranked, key, value, priority);
        }
        ranked
            .into_iter()
    ...
    fn insert_var_option<'b>(
        result: &mut BTreeMap<String, (AquaVarOptionPriority, &'b toml::Value)>,
        key: String,
        value: &'b toml::Value,
        priority: AquaVarOptionPriority,
    ) {
        if result
            .get(&key)
            .is_none_or(|(existing_priority, _)| priority > *existing_priority)
        {
            result.insert(key, (priority, value));
        }
    }
    ...
    fn insert_nested_var_options<'b>(
        ...
        for (key, value) in table {
            Self::insert_var_option(result, key.clone(), value, AquaVarOptionPriority::Nested);
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
enum AquaVarOptionPriority {
    Prefixed,
    Plain,
    Nested,
}
