...
impl Backend for SPMBackend {
    ...
    async fn _list_remote_versions(&self) -> eyre::Result<Vec<String>> {
        ...
        Ok(github::list_releases(repo.shorthand.as_str()).await?
            .into_iter()
            .map(|r| r.tag_name)
            .rev()
            .collect())
    ...
    async fn install_version_(
        ...
        let revision = if tv.version == "latest" {
            self.latest_stable_version().await?
        ...
        for executable in executables {
            let exe_path = self.build_executable(&executable, &repo_dir, ctx, &tv).await?;
