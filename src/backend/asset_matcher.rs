...
/// An asset called nothing but `source` is the release's own source archive, not a build of it.
///
...
///
/// `is_package_or_installer_asset` cannot catch it: that excludes by extension, and a source
/// tarball shares `.tar.gz` with the real ones. The name is the only thing that separates them.
///
/// **Deliberately one exact stem, not a family.** `-source`/`_source` suffixes were tried and
/// rejected as too easy to fire on a real asset; a file called plainly `source.tar.gz` — which is
/// what cargo-dist emits — cannot reasonably be a binary distribution. The trade is that a source
/// archive named some other way is still not excluded.
fn is_source_archive_asset(asset: &str) -> bool {
    asset_name_stem(asset) == "source"
}
...
abc123def456abc123def456abc123def456abc123def456abc123def456abcd  tool-darwin.tar.gz"#;
    ...
    /// The assets of `libsql-server-v0.24.32`, verbatim. Held here rather than fetched: a test that
    /// reads a third party's release list fails when that party changes it, which is what moved
    /// `test_tool_stub_basic` off `stylelint` the same week this was written.
    ...
    #[test]
    fn only_an_asset_called_nothing_but_source_is_excluded() {
        // The rule is one exact stem. Everything in the second list is kept, including the two an
        // earlier `-source`/`_source` form of this rule would have dropped -- which is the whole
        // reason it is written this narrowly: an asset whose *name ends in* "source" can plausibly
        // be a real build, and one called plainly `source` cannot.
        for name in ["source.tar.gz", "source.zip", "SOURCE.TAR.GZ"] {
