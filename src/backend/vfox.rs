...
impl Backend for VfoxBackend {
    ...
    async fn install_version_(
        ...
        vfox.install(&self.pathname, &tv.version, tv.install_path()).await?;
        Ok(tv)
    }
    ...
    async fn exec_env(&self, _config: &Config, _ts: &Toolset, tv: &ToolVersion) -> eyre::Result<EnvMap> {
        Ok(self
            ._exec_env(tv)
            .await?
            .into_iter()
    ...
    async fn _exec_env(&self, tv: &ToolVersion) -> eyre::Result<BTreeMap<String, String>> {
        ...
        cache
            .get_or_try_init_async(async || {
                ...
                Ok(vfox.env_keys(&self.pathname, &tv.version).await?
                    .into_iter()
