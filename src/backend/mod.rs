...
pub trait BackendImpl: Debug + Send + Sync {
    ...
    /// Backend-specific override for `list_remote_versions_with_info`.
    /// Most backends should override `_list_remote_versions` instead.
    async fn list_remote_versions_with_info_impl(
        &self,
        config: &Arc<Config>,
    ) -> eyre::Result<Vec<VersionInfo>> {
        list_remote_versions_with_info_cached(self, config).await
    }
    ...
    async fn latest_stable_version(&self, config: &Arc<Config>) -> eyre::Result<Option<String>> {
        latest_version_for_query(self, config, "latest", None).await
    }
    ...
    // ========== Lockfile Metadata Fetching Hooks ==========
    ...
    /// Backend-specific implementation for resolving platform-specific lock information.
    async fn resolve_lock_info_impl(
        ...
        if let Some(tarball_url) = self.get_tarball_url(tv, target).await? {
            return resolve_lock_info_from_tarball(self, &tarball_url, tv, target).await;
        }

        if let Some(release_info) = self.get_github_release_info(tv, target).await? {
            return resolve_lock_info_from_github_release(self, &release_info, tv, target).await;
        }

        resolve_lock_info_fallback(self, tv, target).await
    }
}
...
pub(crate) fn get_all_dependencies<B: BackendImpl + ?Sized>(
    ...
    let all_fulls = backend.ba().all_fulls();
    ...
    let mut deps: Vec<&str> = backend.get_dependencies()?;
    if optional {
        deps.extend(backend.get_optional_dependencies()?);
    }
    let mut deps: IndexSet<_> = deps.into_iter().map(BackendArg::from).collect();
    if let Some(rt) = REGISTRY.get(backend.ba().short.as_str()) {
        // add dependencies from registry/
        deps.extend(rt.depends.iter().map(BackendArg::from));
    }
    deps.retain(|ba| &**backend.ba() != ba);
    deps.retain(|ba| !all_fulls.contains(&ba.full()));
    for ba in deps.clone() {
        if let Ok(backend) = ba.backend() {
            deps.extend(get_all_dependencies(backend.as_ref(), optional)?);
        }
    }
    Ok(deps)
}

pub(crate) async fn list_remote_versions<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
) -> eyre::Result<Vec<String>> {
    Ok(list_remote_versions_with_info(backend, config)
        .await?
        .into_iter()
        .map(|v| v.version)
        .collect())
...
/// Results are cached. Backends can override `list_remote_versions_with_info_impl`
/// to bypass caching, or `_list_remote_versions` to provide timestamp information.
pub(crate) async fn list_remote_versions_with_info<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
) -> eyre::Result<Vec<VersionInfo>> {
    backend.list_remote_versions_with_info_impl(config).await
}

async fn list_remote_versions_with_info_cached<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
) -> eyre::Result<Vec<VersionInfo>> {
    let remote_versions = backend.get_remote_version_cache();
    ...
    let use_versions_host = if !versions_host_applies {
        ...
        false
    } else if let Some(plugin) = backend.plugin()
        && let Ok(Some(remote_url)) = plugin.get_remote_url()
    {
        ...
        let shorthand_remote = REGISTRY
            ...
            .unwrap_or_default();
        let matches = normalized_remote == normalize_remote(&shorthand_remote).unwrap_or_default();
        if !matches {
            trace!(
    ...
    let versions = remote_versions
        .get_or_try_init_async(|| async {
            ...
            let versions = backend
                ...
                .collect_vec();
            if versions.is_empty() && backend.get_type() != BackendType::Http {
...
pub(crate) fn list_installed_versions<B: BackendImpl + ?Sized>(backend: &B) -> Vec<String> {
    install_state::list_versions(&backend.ba().short)
}

pub(crate) fn is_version_installed<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    tv: &ToolVersion,
    check_symlink: bool,
    ...
    let check_path = |install_path: &Path, check_symlink: bool| {
        ...
        let is_not_incomplete = !incomplete_file_path(tv).exists();
...
pub(crate) async fn is_version_outdated<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    tv: &ToolVersion,
) -> bool {
    let latest = match tv.latest_version(config).await {
        ...
        Err(e) => {
            warn!(
                ...
                backend.ba().to_string(),
                e
            );
            return false;
        }
    };
    !is_version_installed(backend, config, tv, true) || is_outdated_version(&tv.version, &latest)
}

pub(crate) fn symlink_path(
    _backend: &(impl BackendImpl + ?Sized),
    tv: &ToolVersion,
) -> Option<PathBuf> {
...
pub(crate) fn create_symlink<B: BackendImpl + ?Sized>(
    backend: &B,
    version: &str,
    target: &Path,
) -> Result<Option<(PathBuf, PathBuf)>> {
    let link = backend.ba().installs_path.join(version);
    if link.exists() {
        return Ok(None);
    }
...
pub(crate) fn list_installed_versions_matching<B: BackendImpl + ?Sized>(
    backend: &B,
    query: &str,
) -> Vec<String> {
    let versions = list_installed_versions(backend);
    backend.fuzzy_match_filter(versions, query)
}

pub(crate) async fn list_versions_matching<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    query: &str,
) -> eyre::Result<Vec<String>> {
    let versions = list_remote_versions(backend, config).await?;
    Ok(backend.fuzzy_match_filter(versions, query))
}
...
pub(crate) async fn list_versions_matching_with_opts<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    query: &str,
    ...
    let versions = match before_date {
        Some(before) => {
            ...
            let versions_with_info = list_remote_versions_with_info(backend, config).await?;
        ...
        None => list_remote_versions(backend, config).await?,
    };
    Ok(backend.fuzzy_match_filter(versions, query))
}

pub(crate) async fn latest_version_for_query<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    query: &str,
    before_date: Option<Timestamp>,
) -> eyre::Result<Option<String>> {
    let mut matches = list_versions_matching_with_opts(backend, config, query, before_date).await?;
...
/// cutoff is active.
pub(crate) async fn latest_version<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    query: Option<String>,
    before_date: Option<Timestamp>,
) -> eyre::Result<Option<String>> {
    let before_date = effective_latest_before_date(backend, config, before_date).await?;
    match query.as_deref() {
        Some("latest") | None => match before_date {
            Some(before) => latest_version_for_query(backend, config, "latest", Some(before)).await,
            None => backend.latest_stable_version(config).await,
        },
        Some(query) => latest_version_for_query(backend, config, query, before_date).await,
    }
}

pub(crate) fn latest_installed_version<B: BackendImpl + ?Sized>(
    backend: &B,
    query: Option<String>,
) -> eyre::Result<Option<String>> {
    match query {
        Some(query) => {
            let matches = list_installed_versions_matching(backend, &query);
            Ok(find_match_in_list(&matches, &query))
        }
        None => {
            let installed_symlink = backend.ba().installs_path.join("latest");
...
pub(crate) async fn get_version_info<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    version: &str,
) -> Option<VersionInfo> {
    let versions = match list_remote_versions_with_info(backend, config).await {
        Ok(v) => v,
        Err(_) => return None,
    };
...
pub(crate) async fn is_rolling_version_outdated<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    version: &str,
) -> bool {
    ...
    let version_info = match get_version_info(backend, config, version).await {
    ...
    let stored_checksum = install_state::read_checksum(&backend.ba().short, version);
...
#[allow(dead_code)]
pub(crate) async fn warn_if_dependencies_missing<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
) -> eyre::Result<()> {
    let deps = get_all_dependencies(backend, false)?
        .into_iter()
        .filter(|ba| &**backend.ba() != ba)
        .map(|ba| ba.short)
        .collect::<HashSet<_>>();
    if !deps.is_empty() {
        trace!("Ensuring dependencies installed for {}", backend.id());
...
pub(crate) fn purge<B: BackendImpl + ?Sized>(
    backend: &B,
    pr: &dyn SingleReport,
) -> eyre::Result<()> {
    remove_all_with_progress(&backend.ba().installs_path, pr)?;
    remove_all_with_progress(&backend.ba().cache_path, pr)?;
    remove_all_with_progress(&backend.ba().downloads_path, pr)?;
    Ok(())
}
...
pub(crate) async fn idiomatic_filenames<B: BackendImpl + ?Sized>(
    backend: &B,
) -> Result<Vec<String>> {
    let mut filenames = backend._idiomatic_filenames().await?;
    if let Some(rt) = REGISTRY.get(backend.id()) {
...
pub(crate) async fn parse_idiomatic_file<B: BackendImpl + ?Sized>(
    backend: &B,
    path: &Path,
) -> eyre::Result<Vec<String>> {
    ...
    backend._parse_idiomatic_file(path).await
}

pub(crate) async fn install_version<B: BackendImpl + ?Sized>(
    backend: &B,
    ctx: InstallContext,
    mut tv: ToolVersion,
    ...
    if ctx.locked && !tv.request.source().is_tool_stub() && backend.supports_lockfile_url() {
    ...
    if ctx.dry_run {
        ...
        if is_version_installed(backend, &ctx.config, &tv, true) {
            ctx.pr
    ...
    let will_uninstall = ctx.force && is_version_installed(backend, &ctx.config, &tv, true);

    // Query backend for operation count and set up progress tracking
    let install_ops = backend.install_operation_count(&tv, &ctx).await;
    ...
    if will_uninstall {
        uninstall_version(backend, &ctx.config, &tv, ctx.pr.as_ref(), false).await?;
        ctx.pr.next_operation();
    } else if is_version_installed(backend, &ctx.config, &tv, true) {
        return Ok(tv);
    }
    ...
    if is_version_installed(backend, &ctx.config, &tv, true) && !ctx.force {
        return Ok(tv);
    }

    create_install_dirs(&tv)?;

    let old_tv = tv.clone();
    let tv = match backend.install_version_(&ctx, tv).await {
        Ok(tv) => tv,
        Err(e) => {
            cleanup_install_dirs_on_error(&old_tv);
    ...
    if install_path.starts_with(*dirs::INSTALLS) {
        install_state::write_backend_meta(backend.ba())?;
        ...
        if let Some(installs_dir) = install_path.parent().and_then(|p| p.parent()) {
            ...
            install_state::write_backend_meta_to(backend.ba(), &manifest)?;
        }
    }

    cleanup_install_dirs(&tv);
    ...
    let incomplete_path = incomplete_file_path(&tv);
    ...
    if let Some(script) = tv.request.options().get("postinstall") {
        ...
        run_postinstall_hook(backend, &ctx, &tv, script).await?;
    }
    ctx.pr.finish_with_message("installed".to_string());
    Ok(tv)
}

async fn run_postinstall_hook<B: BackendImpl + ?Sized>(
    backend: &B,
    ctx: &InstallContext,
    tv: &ToolVersion,
    script: &str,
    ...
    let mut env_vars = backend.exec_env(&ctx.config, &ctx.ts, tv).await?;
    ...
    let bin_paths = backend.list_bin_paths(&ctx.config, tv).await?;
...
pub(crate) async fn uninstall_version<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    tv: &ToolVersion,
    pr: &dyn SingleReport,
    ...
    if !dryrun {
        backend.uninstall_version_impl(config, pr, tv).await?;
    }
    let rmdir = |dir: &Path| {
        if dryrun {
            if dir.exists() {
...
pub(crate) async fn which<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    tv: &ToolVersion,
    bin_name: &str,
...
fn create_install_dirs(tv: &ToolVersion) -> eyre::Result<()> {
    ...
    File::create(incomplete_file_path(tv))?;
    Ok(())
}

fn cleanup_install_dirs_on_error(tv: &ToolVersion) {
    if !Settings::get().always_keep_install {
        let _ = remove_all_with_warning(tv.install_path());
        // Clean up the incomplete marker from cache
        let _ = file::remove_file(incomplete_file_path(tv));
...
fn cleanup_install_dirs(tv: &ToolVersion) {
    if !Settings::get().always_keep_download {
        let _ = remove_all_with_warning(tv.download_path());
    }
}

pub(crate) fn incomplete_file_path(tv: &ToolVersion) -> PathBuf {
    install_state::incomplete_file_path(&tv.ba().short, &tv.tv_pathname())
}

pub(crate) async fn path_env_for_cmd<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    tv: &ToolVersion,
) -> Result<OsString> {
    let path = backend
        ...
        .into_iter()
        .chain(
            dependency_toolset(backend, config)
                .await?
                .list_paths(config)
                .await,
        )
...
pub(crate) async fn dependency_toolset<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
) -> eyre::Result<Toolset> {
    let dependencies = get_all_dependencies(backend, true)?
        .into_iter()
        .map(|ba| ba.short)
        .collect();
...
pub(crate) async fn dependency_which<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    bin: &str,
) -> Option<PathBuf> {
    ...
    let Ok(ts) = dependency_toolset(backend, config).await else {
        return None;
    };
    let (b, tv) = ts.which(config, bin).await?;
    which(b.as_ref(), config, &tv, bin).await.ok().flatten()
}
...
pub(crate) async fn warn_if_dependency_missing<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    program: &str,
    provided_by: &[&str],
    ...
    let found = if dependency_which(backend, config, program).await.is_some() {
        true
    } else if cfg!(windows) {
        ...
        for ext in &settings.windows_executable_extensions {
            if dependency_which(backend, config, &format!("{}.{}", program, ext))
                .await
                .is_some()
            {
                found = true;
    ...
    if !found {
        ...
        if let Ok(ts) = dependency_toolset(backend, config).await
            && ts
                .versions
                .keys()
...
pub(crate) async fn dependency_env<B: BackendImpl + ?Sized>(
    backend: &B,
    config: &Arc<Config>,
    ...
    let mut env = dependency_toolset(backend, config)
        .await?
...
pub(crate) fn verify_checksum<B: BackendImpl + ?Sized>(
    backend: &B,
    ctx: &InstallContext,
    tv: &mut ToolVersion,
    file: &Path,
...
pub(crate) async fn resolve_lock_info<B: BackendImpl + ?Sized>(
    backend: &B,
    tv: &ToolVersion,
    target: &PlatformTarget,
) -> Result<PlatformInfo> {
    backend.resolve_lock_info_impl(tv, target).await
}
...
pub(crate) async fn resolve_lock_info_from_tarball<B: BackendImpl + ?Sized>(
    _backend: &B,
    tarball_url: &str,
    _tv: &ToolVersion,
    _target: &PlatformTarget,
...
pub(crate) async fn resolve_lock_info_from_github_release<B: BackendImpl + ?Sized>(
    _backend: &B,
...
pub(crate) async fn resolve_lock_info_fallback<B: BackendImpl + ?Sized>(
    _backend: &B,
    _tv: &ToolVersion,
    _target: &PlatformTarget,
) -> Result<PlatformInfo> {
...
mod latest_version_tests {
    ...
    #[async_trait]
    impl crate::backend::BackendImpl for LatestBackend {
        fn ba(&self) -> &Arc<BackendArg> {
            &self.ba
        }
    ...
    async fn test_explicit_latest_uses_latest_stable_version() {
        ...
        assert_eq!(
            latest_version(&backend, &config, Some("latest".to_string()), None)
                .await
                .unwrap()
                .as_deref(),
            Some("9.9.9")
        ...
        assert_eq!(
            latest_version(&backend, &config, None, None)
                .await
                .unwrap()
                .as_deref(),
            Some("9.9.9")
    ...
    async fn test_date_filtered_latest_bypasses_latest_stable_version() {
        ...
        assert_eq!(
            latest_version(&backend, &config, Some("latest".to_string()), Some(before))
                .await
                .unwrap()
                .as_deref(),
            Some("1.0.0")
