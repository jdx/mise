...
impl Backend for AsdfBackend {
    ...
    async fn latest_stable_version(&self) -> Result<Option<String>> {
        ...
        timeout::run_with_timeout_async(
            || async {
                if !this.plugin.has_latest_stable_script() {
                    return this.latest_version(Some("latest".into())).await;
                }
                this.latest_stable_cache
                    .get_or_try_init(|| this.plugin.fetch_latest_stable())
                    .wrap_err_with(|| {
                        eyre!(
                            "Failed fetching latest stable version for plugin {}",
                            style(&this.name).blue().for_stderr(),
                        )
                    })
                    .cloned()
            },
    ...
    async fn uninstall_version_impl(&self, pr: &Box<dyn SingleReport>, tv: &ToolVersion) -> Result<()> {
        if self.plugin_path.join("bin/uninstall").exists() {
            self.script_man_for_tv(tv).await?
    ...
    async fn list_bin_paths(&self, tv: &ToolVersion) -> Result<Vec<PathBuf>> {
        Ok(self
            .cache
            .list_bin_paths(self, tv, async || self.fetch_bin_paths(tv).await).await?
            .into_iter()
            .map(|path| tv.install_path().join(path))
            .collect())
    }

    async fn exec_env(&self, config: &Config, ts: &Toolset, tv: &ToolVersion) -> eyre::Result<EnvMap> {
        ...
        self.cache
            .exec_env(config, self, tv, async || self.fetch_exec_env(ts, tv).await).await
    }
}
