...
impl CondaBackend {
    ...
    #[cfg(target_os = "macos")]
    fn fix_library_paths(&self, ctx: &InstallContext, install_dir: &Path) -> Result<()> {
        ...
        let files = self.find_macho_files(install_dir)?;
        ...
        for file_path in &files {
            ...
            for line in deps.lines().skip(1) {
                ...
                let line = line.trim();
                if let Some(old_path) = Self::extract_build_path(line) {
    ...
    fn fix_library_paths(&self, ctx: &InstallContext, install_dir: &Path) -> Result<()> {
        ...
        // Find all ELF files
        let files = self.find_elf_files(install_dir)?;
    ...
    fn fix_library_paths(&self, _ctx: &InstallContext, _install_dir: &Path) -> Result<()> {
        Ok(())
    }

    /// Find all Mach-O files (executables and dylibs) in a directory
    #[cfg(target_os = "macos")]
    fn find_macho_files(&self, dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    #[cfg(target_os = "linux")]
    fn find_elf_files(&self, dir: &Path) -> Result<Vec<PathBuf>> {
        let mut files = Vec::new();
        for entry in WalkDir::new(dir).into_iter().filter_map(|e| e.ok()) {
            let path = entry.path();
...
/// Check if a file is an ELF binary (Linux)
#[cfg(target_os = "linux")]
...
impl Backend for CondaBackend {
    ...
    async fn install_version_(
        ...
        self.fix_library_paths(ctx, &install_path)?;
