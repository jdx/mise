use crate::config::{env_directive::EnvResults, Config};
...
impl EnvResults {
        ...
        PT: FnMut(String) -> Result<String>,
    {
        ...
        if let Ok(raw) = file::read_to_string(p) {
            ...
            if !f.sops.is_empty() {
                let raw = sops::decrypt::<_, JsonFileFormat>(config, &raw, parse_template, "json").await?;
        ...
        PT: FnMut(String) -> Result<String>,
    {
        ...
        if let Ok(raw) = file::read_to_string(p) {
            ...
            if !f.sops.is_empty() {
                let raw = sops::decrypt::<_, YamlFileFormat>(config, &raw, parse_template, "yaml").await?;
