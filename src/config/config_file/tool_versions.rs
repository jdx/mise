...
impl ToolVersions {
    ...
    fn add_version(&self, fa: &BackendArg, version: String) {
        let mut plugins = self.plugins.lock().unwrap();
        get_or_create_plugin(&mut plugins, fa).versions.push(version);
    }
...
fn get_or_create_plugin<'a>(plugins: &'a mut IndexMap<BackendArg, ToolVersionPlugin>, fa: &BackendArg) -> &'a mut ToolVersionPlugin {
    plugins
        .entry(fa.clone())
