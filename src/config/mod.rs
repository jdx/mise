...
use crate::task::Task;
use crate::toolset::{ToolRequestSet, ToolRequestSetBuilder, ToolVersion, Toolset, ToolsetBuilder, install_state};
use crate::ui::style;
...
impl Config {
    ...
    pub async fn load() -> Result<Self> {
        ...
        let vars_results = load_vars(&config).await?;
...
async fn load_global_tasks(config: &Config) -> Vec<Task> {
    ...
    for cf in global_config_files {
        let cf = cf.clone();
        jset.spawn(async move { load_config_and_file_tasks(cf).await });
    }
