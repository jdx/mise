# Dotfile Edits <Badge type="warning" text="experimental" />

Where [dotfile file entries](/system-files.html) manage whole files,
`[dotfiles]` can also manage one small piece of a file something else owns —
the `mise activate` line in your shell rc, an entry in `/etc/hosts`. Edit
entries are keyed by target path plus an id naming each edit within the file:
...
A table value spells out the operation:
...
```toml
[dotfiles]
"~/.zshrc/activate" = { block = "..." }                                  # inline
"~/.zshrc/aliases" = { source = "snippets/aliases.sh" }                   # from a file, relative to this config
"~/.zshrc/prompt" = { source = "snippets/prompt.tmpl", template = "tera" } # rendered with the template engine
```
...
[Dotfile Files](/system-files.html) to own the whole file instead.
...
`mise dotfiles status` (and a real install) evaluates templates — including
...
- **Declarative and additive** — entries merge across the
  ...
  same id, exactly like [Dotfile Files](/system-files.html) overrides by
  target.
- **Manual application only** — nothing is written implicitly. Only
  `mise dotfiles install` (or [`mise bootstrap`](/cli/bootstrap.html)) applies
  edits.
...
- **Surgical** — edits never create conflicts with existing content and
  ...
  whatever the link points at (often a `[dotfiles]` source), so point
...
```sh
mise dotfiles status            # shows edit state: applied/missing/differs
mise dotfiles status --missing  # exit 1 if anything is missing (CI check)

mise dotfiles install           # apply files and edits (prompts first)
mise dotfiles install --dry-run # print what would be done
mise dotfiles install --yes     # skip the confirmation prompt
```

`mise dotfiles status` reports each edit as `applied`, `missing` (no markers
