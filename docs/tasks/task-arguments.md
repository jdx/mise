...
Tasks that bypass usage parsing with `raw_args` or an explicit `-- --help` or
`-- -h` passthrough retain their inherited environment. No usage parser runs on
these paths, so an inherited value cannot masquerade as an omitted parser
result. This compatibility exception preserves raw proxy behavior; the task or
downstream command is responsible for interpreting its arguments and
environment. To intentionally inherit a value in a normally parsed task, use a
separately named environment variable, optionally as an argument input with
`env=`:
