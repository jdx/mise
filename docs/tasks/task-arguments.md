...
`-- -h` passthrough retain their inherited environment. To intentionally
inherit a value, use a separately named environment variable, optionally as an
argument input with `env=`:
