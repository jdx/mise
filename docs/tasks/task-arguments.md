...
For normal task execution, mise treats the `usage_*` variables as internal
outputs that describe only the current task invocation. It clears values
inherited from a parent process before discovering the usage spec or parsing
arguments, even when a task does not declare a usage spec. This prevents an
omitted argument from accidentally reusing a value generated by a parent task.
Tasks that opt out of usage parsing with `raw_args` retain their inherited
environment. No usage parser runs for these tasks, so an inherited value cannot
masquerade as an omitted parser result. This compatibility exception preserves
raw proxy behavior; the task or downstream command is responsible for
interpreting its arguments and environment. To intentionally inherit a value
in a normally parsed task, use a separately named environment variable,
optionally as an argument input with `env=`:
