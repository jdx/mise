...
as configuring the script like any other, under either spelling — its metadata
applies and the script keeps running. When no config declares `task_config.includes` and the directory
...
higher-precedence config — the same requirement a command has to meet to replace
an executable script under its full name.
