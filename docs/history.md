...
rather than enumerating it, which is only a speed-up: the files under it
would each have been excluded anyway.
...
in force when the checkpoint was saved — the expanded pattern with its
negation kept as a separate field, never as a leading `!` — and a
rollback reads that rather than expanding again in whatever environment
it is running in, where the same variable may be unset, empty, or simply
different by then. Where it cannot tell what a checkpoint covered, it
leaves those paths alone and says so rather than removing a file the
snapshot never held.
