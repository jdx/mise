...
does in `.gitignore`: `"/*.md"` skips only the markdown files at the root.
Either kind matching a directory skips everything under it. Use `**` to match across directories:
...
a leading `/` selects from its top level only: `/*.log` matches
`~/.codex/a.log` but not `~/.codex/logs/b.log`. Use
