...
For `npm:`, `pipx:`, and `conda:` tools, the same cutoff is also forwarded to dependency
resolution during install. Refer to the [npm backend docs](/dev-tools/backends/npm.html),
[pipx backend docs](/dev-tools/backends/pipx.html), and
[conda backend docs](/dev-tools/backends/conda.html) for backend support details.
