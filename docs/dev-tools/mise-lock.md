...
```text
mise.lock
.mise/locks/pipx-black/24.10.0/pyproject.toml
.mise/locks/pipx-black/24.10.0/uv.lock
...
version = "24.10.0"
backend = "pypi:black"
uv = { path = ".mise/locks/pipx-black/24.10.0", digest = "sha256:…" }
```
...
stays unchanged when other variants are added. Removing an entry removes its
unreferenced sidecar directory on the next save.
...
After an external tool or a person edits a sidecar, ordinary `mise install`
validates the graph and installs it under the identity of its actual bytes, then
updates the recorded digest through auto-locking. `mise lock` also accepts valid
...
regenerated by `mise lock`; locked installation fails clearly.
