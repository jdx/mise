...
By default, external `cargo-binstall` may use the third-party cargo-quickinstall artifact host.
This is separate from crate-author GitHub releases and artifacts declared in
`package.metadata.binstall`. Set `cargo.binstall_quickinstall = false` to disable only that
strategy. This setting does not affect mise's native `cargo.binstall_native` path, which does not
use quickinstall. Set `cargo.binstall = false` to disable binstall entirely.
