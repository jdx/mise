...
The target hostname is available as `MISE_CREDENTIAL_HOST`, and the provider name (`forgejo`) is available as `MISE_CREDENTIAL_PROVIDER`. For sh-compatible shells (`ash`, `bash`, `dash`, `ksh`, `sh`, and `zsh`), the hostname is also passed as `$1`; other shells should use `MISE_CREDENTIAL_HOST` instead.
