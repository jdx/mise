...
`mise skills ls` and `mise skills sync` report a skill a packslip declares that
is not in the install, by name and with the reason:
[`skills.fetch`](/configuration/settings.html#skills.fetch) is off, the skill is
generated by an `exec` command and
[`packslip.exec`](/configuration/settings.html#packslip.exec) is off, or the
download failed at install time and reinstalling the tool will fetch it again.
