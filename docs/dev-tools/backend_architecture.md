...
All backends implement a common hook interface (called a "trait" in Rust), while shared lifecycle operations are exposed through the blanket-implemented `Backend` operations trait:
...
// Callers use shared operations through method syntax:
