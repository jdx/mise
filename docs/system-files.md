# Dotfile Files <Badge type="warning" text="experimental" />

mise can place config files (dotfiles) at target paths via the `[dotfiles]`
...
[Dotfile Edits](/system-edits.html).
...
`mise dotfiles install --force` to replace them.
...
```sh
mise dotfiles status            # shows file state: applied/missing/differs
mise dotfiles status --missing  # exit 1 if anything is out of sync (CI check)

mise dotfiles install           # apply files (prompts first)
mise dotfiles install --dry-run # print what would be done
mise dotfiles install --yes     # skip the confirmation prompt
mise dotfiles install --force   # also replace conflicting files
```

`mise dotfiles status` reports each entry as `applied`, `missing`, `differs`
