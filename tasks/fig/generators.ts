...
const getInstalledTools = async (executeShellCommand: Fig.ExecuteCommandFunction) => {
  const {stdout} = await executeShellCommand({
    command: 'sh', args: ['-c', 'mise ls --installed']
  })
...
const allPluginsGenerator: Fig.Generator = singleCmdNewLineGenerator('mise plugins --all')
...
const searchBackend = async (backend: string, context: string[], 
    executeShellCommand: Fig.ExecuteCommandFunction, 
    shellContext: Fig.GeneratorContext): Promise<Fig.Suggestion[]> => {
  const customContext = context;
  customContext[context.length - 1] = customContext[context.length - 1].replace(`${backend}:`, '')
  switch (backend) {
    case 'npm':
      return (await (createNpmSearchHandler()(context, executeShellCommand, shellContext)))
    case 'cargo':
      //return [{name: customContext[context.length - 1]}]
      return (await (createCargoSearchGenerator.custom(customContext, executeShellCommand, shellContext)))
    case 'asdf':
      const { stdout } = await executeShellCommand({
        command: 'sh', args: ['-c', 'mise registry']
      })
      return [...new Set(stdout.split('\n').map(l => {
        const tokens = l.split(/\s+/)
...
const toolVersionGenerator: Fig.Generator = {
  trigger: (newToken: string, oldToken: string): boolean => {
    return (backendSepInStr(newToken) && !backendSepInStr(oldToken)) || 
      (atsInStr(newToken) && !atsInStr(oldToken))
  },
  getQueryTerm: '@',

  custom: async (context: string[],
    executeShellCommand: Fig.ExecuteCommandFunction,
    ...
    if (backendSepInStr(currentWord)) {
      ...
      return (await searchBackend(backend, context, executeShellCommand, shellContext))
        .map(s => ({...s, name: `${backend}:${s.name}`, displayName: s.name, icon: '📦'}))
      
    } else if (atsInStr(currentWord)) {
      const tool = currentWord.slice(0, currentWord.lastIndexOf('@'))
      const { stdout } = await executeShellCommand({
        command: 'sh', args: ['-c', `mise ls-remote ${tool}`]
      })
      const remote_versions_suggestions = stdout.split('\n').sort((a,b) => compareVersions(b,a)).map(l => ({name: l}))
      const { stdout: aliasStdout } = await executeShellCommand({
        command: 'sh', args: ['-c', `mise alias ls ${tool}`]
      })
  ...
  custom: async (context: string[],
    ...
    if (atsInStr(currentWord)) {
      ...
      */
      ...
      const toolVersions = (toolsVersions[tool] || []) as {name: string, version: string}[]
