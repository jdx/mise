...
impl RegistryFetcher for DefaultRegistryFetcher {
    async fn fetch_registry(&self, package_id: &str) -> Result<RegistryYaml> {
            ...
            && let Some(content) = baked_registry_file(package_id)
        {
            ...
            let content = zstd::stream::decode_all(content)?;
            return Ok(serde_yaml::from_slice(&content)?);
        }
...
mod tests {
    ...
    fn test_baked_registry_package_lookup() {
        ...
        let registry = zstd::stream::decode_all(registry).unwrap();
        let registry = serde_yaml::from_slice::<RegistryYaml>(&registry).unwrap();
