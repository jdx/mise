...
impl FromLua for BackendUninstallResponse {
    fn from_lua(value: Value, _: &Lua) -> std::result::Result<Self, LuaError> {
        match value {
            Value::Table(table) => Ok(BackendUninstallResponse {
                success: table.get::<bool>("success")?,
                message: table.get::<Option<String>>("message")?,
            }),
            _ => Err(LuaError::FromLuaConversionError {
                from: value.type_name(),
                to: "BackendUninstallResponse".to_string(),
