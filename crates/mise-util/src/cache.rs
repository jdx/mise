//! On-disk caches keyed by mise build and platform, and pruning of cache directories.
...
pub fn effective_remote_cache_mode(configured: CacheRemoteMode) -> Option<CacheRemoteMode> {
...
static BASE_CACHE_KEYS: std::sync::OnceLock<Vec<String>> = std::sync::OnceLock::new();
...
pub fn prepare_auto_prune_root(cache_dir: &Path, age: Duration) -> Result<bool> {
    if !cache_dir.exists() {
        return Ok(false);
    }
...
pub fn prune(dir: &Path, opts: &PruneOptions) -> Result<PruneResults> {
