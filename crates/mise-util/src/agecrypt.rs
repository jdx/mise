...
pub fn unusable_identity_hint(unusable: &[(PathBuf, UnusableSshIdentity)]) -> String {
...
pub const MAX_ENCRYPTED_BYTES: u64 = 256 * 1024 * 1024;
pub const MAX_PLAINTEXT_BYTES: u64 = 1024 * 1024 * 1024;

pub fn read_bounded(reader: impl Read, limit: u64) -> std::io::Result<Vec<u8>> {
...
pub async fn decrypt_bytes_mode(
    ciphertext: &[u8],
    interactive: bool,
...
pub async fn default_recipient_strings() -> Result<Vec<String>> {
...
/// `.pub` file must exist.
pub async fn ssh_public_key_for_private(path: &Path) -> Result<String> {
...
pub async fn load_all_identities() -> LoadedIdentities {
...
pub fn decrypt_sync(ciphertext: &[u8], interactive: bool) -> Result<Vec<u8>> {
...
pub fn identity_paths() -> Vec<PathBuf> {
...
mod tests {
    ...
    async fn test_encrypt_bytes_round_trips_and_compresses() -> Result<()> {
        ...
        let mut vars = crate::testing::EnvVarGuard::new();
