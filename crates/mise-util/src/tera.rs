...
impl TeraEngine {
    ...
    pub fn validate_template_syntax(&mut self, name: &str, input: &str) -> TeraResult<()> {
        match self {
...
pub static BASE_CONTEXT: Lazy<Context> = Lazy::new(|| {
    ...
    let mise_env = env::mise_env();
    if !mise_env.is_empty() {
        context.insert("mise_env", mise_env);
    }
...
pub fn get_tera_v2(dir: Option<&Path>) -> Tera {
...
pub fn get_tera_for_oci(dir: Option<&Path>) -> TeraEngine {
...
pub fn get_tera_without_exec(dir: Option<&Path>) -> TeraEngine {
...
pub fn tera1_exec(
    ...
    move |args: &HashMap<String, JsonValue>| -> tera1::Result<JsonValue> {
        ...
        let shell = crate::shells::default_inline_shell(&Settings::get())
            .map_err(|e| tera1_err(e.to_string()))?;
        ...
        let run_once = || -> eyre::Result<String> {
            ...
            Ok(crate::inline_command::optimize_expression(
                ...
                &env_no_shims,
                dir.as_deref(),
                crate::shells::implicit_inline_shell(),
            )
            .read()?)
        };
        Ok(json!(
