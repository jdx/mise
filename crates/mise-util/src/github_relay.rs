...
impl Scope {
    pub fn from_flags(enabled: bool, repositories: &[String], all: bool) -> Result<Option<Self>> {
        if !enabled {
...
pub fn expand_repository(value: &str) -> Result<String> {
    if value.contains(':')
        || value.starts_with(['/', '.', '~'])
...
pub mod unix {
    ...
    impl Relay {
        ...
        pub async fn start(scope: Scope, destination: &str) -> Result<Self> {
    ...
    pub async fn session(socket: &Path, command: Vec<String>) -> Result<()> {
        ...
        drop(guard);
        Err(crate::exit::request(status.code().unwrap_or(255)))
    }
    ...
    pub async fn wait_command(
        ...
        let code = tokio::select! {
            status = child.wait() => {
                ...
                if let Some(signal) = status.signal() {
                    return Err(crate::exit::request(128 + signal));
                }
                return Ok(status);
            },
        ...
        child.kill().await?;
        Err(crate::exit::request(code))
    }

    pub async fn lifecycle<T>(
        ...
        if crate::cancel::is_cancelled() {
            return Err(crate::exit::request(130));
        }
        tokio::select! {
            biased;
            _ = interrupt.recv() => Err(crate::exit::request(130)),
            _ = terminate.recv() => Err(crate::exit::request(143)),
            _ = hangup.recv() => Err(crate::exit::request(129)),
