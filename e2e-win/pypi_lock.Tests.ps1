...
"@ | Add-Content mise.toml
            ...
            $exitCode = $LASTEXITCODE
            $exitCode | Should -Be 0
            (Get-Content mise.lock -Raw) | Should -Match 'path = "\.mise/locks/pypi-mise-lock-cli/1\.0\.0"'
            Test-Path '.mise/locks/pypi-mise-lock-cli/1.0.0/uv.lock' | Should -BeTrue
            $output = mise x --locked -- lock-cli
            $exitCode = $LASTEXITCODE
            $exitCode | Should -Be 0
            $output | Should -Be 'dependency=1.0.0'
            ...
            $exitCode = $LASTEXITCODE
            $exitCode | Should -Be 0
            $output | Should -Be 'dependency=2.0.0'
        }
        finally {
