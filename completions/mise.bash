_mise() {
    ...
    if [[ -z ${_usage_spec_mise_2024_11_21:-} ]]; then
        _usage_spec_mise_2024_11_21="$(mise usage)"
    fi

    COMPREPLY=( $(usage complete-word --shell bash -s "${_usage_spec_mise_2024_11_21}" --cword="$COMP_CWORD" -- "${COMP_WORDS[@]}" ) )
    if [[ $? -ne 0 ]]; then
        unset COMPREPLY
    fi
    return 0
