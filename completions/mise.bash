_mise() {
    ...
    if [[ -z ${_usage_spec_mise_2024_12_0:-} ]]; then
        _usage_spec_mise_2024_12_0="$(mise usage)"
    fi

    # shellcheck disable=SC2207
    COMPREPLY=( $(usage complete-word --shell bash -s "${_usage_spec_mise_2024_12_0}" --cword="$COMP_CWORD" -- "${COMP_WORDS[@]}" ) )
    if [[ $? -ne 0 ]]; then
        unset COMPREPLY
    fi
    return 0
