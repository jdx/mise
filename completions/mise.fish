...
set _usage_spec_mise_2026_8_10 'name mise
bin mise
...
{%- elif path is starting_with(pat="bootstrap/packages/brew") -%}
{%- set path = "system/brew/mod.rs" -%}
{%- elif path == "bootstrap/dotfiles/add" -%}
{%- set path = "dotfiles/add.rs" -%}
{%- elif path == "bootstrap/dotfiles/apply" -%}
{%- set path = "dotfiles/apply.rs" -%}
{%- elif path == "bootstrap/dotfiles/edit" -%}
{%- set path = "dotfiles/edit.rs" -%}
{%- elif path == "bootstrap/dotfiles/status" -%}
{%- set path = "dotfiles/status.rs" -%}
{%- elif path == "bootstrap/dotfiles/unapply" -%}
{%- set path = "dotfiles/unapply.rs" -%}
{%- elif path is starting_with(pat="bootstrap") -%}
...
flag "-q --quiet" help="Suppress non-error messages" global=#true env=MISE_QUIET {
...
case $cur in
  *@*)
    ...
    ;;
  *)
    plugins=$(mise ls --installed | awk \'{print $1}\' | uniq | sed \'1!G;h;$!d\')
    for plugin in $plugins; do
      echo "$plugin@"
    done
    ;;
...
"""#
    ...
    cmd set help="Add/update an alias for a tool/backend" effect=write {
        alias add create
        long_help #"""
...
"""#
    ...
    cmd __apply-account-plan hide=#true effect=destructive
    cmd __apply-service-plan hide=#true effect=destructive
    cmd __apply-firewall-plan hide=#true effect=destructive
    cmd __apply-system-plan hide=#true effect=destructive
    cmd __inspect-system-files hide=#true effect=read
    cmd __inspect-firewall-plan hide=#true effect=read
    cmd accounts subcommand_required=#true help="Manage Linux users and groups from `[bootstrap.users]` and `[bootstrap.groups]`" effect=read {
        ...
        cmd status help="Show configured Linux user and group state" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 when any account is not converged"
        }
    }
    cmd compose subcommand_required=#true help="Manage Docker Compose projects from `[bootstrap.compose]`" effect=read {
        ...
        cmd status help="Show configured Docker Compose project state" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 when any Compose project is not converged"
        }
    }
    cmd dotfiles subcommand_required=#true help="Manage dotfiles from `[dotfiles]`" effect=read {
        cmd add help="Add or update dotfiles in `[dotfiles]`" effect=write {
            long_help #"""
...
"""#
            ...
            flag "-y --yes" help="Skip the confirmation prompt"
            arg <TARGET>… help="Targets to add or update" var=#true
        }
        cmd apply help="Apply dotfiles from `[dotfiles]`" effect=write {
            long_help #"""
...
"""#
        ...
        cmd unapply help="Remove dotfiles applied from `[dotfiles]`" effect=destructive {
            long_help #"""
...
"""#
            ...
            flag "-n --dry-run" help="Print the actions that would run without writing anything"
            flag "-y --yes" help="Skip the confirmation prompt"
            arg "[TARGET]…" help="Only unapply these targets" required=#false var=#true
        }
    }
    cmd files subcommand_required=#true help="Manage privileged files and directories from `[bootstrap.files]` and `[bootstrap.directories]`" effect=read {
        ...
        cmd status help="Show configured privileged file and directory state" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 when any resource is not converged"
            flag --prompt-secrets help="Prompt securely for missing bootstrap secret inputs"
        }
    }
    cmd firewall subcommand_required=#true help="Manage the Linux host firewall from `[bootstrap.linux.firewall]`" effect=read {
        ...
        cmd status help="Show configured Linux host firewall state" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 when the firewall is not converged"
        }
    }
    ...
    cmd linux subcommand_required=#true help="Manage Linux bootstrap config from `[bootstrap.linux]`" effect=read {
...
"""#
        ...
        cmd import help="Import installed system packages into `[bootstrap.packages]`" effect=write {
            long_help #"""
...
"""#
        ...
        cmd prune help="Prune installed system packages no longer declared in `[bootstrap.packages]`" effect=destructive {
            long_help #"""
...
"""#
        ...
        cmd upgrade help="Upgrade installed bootstrap packages from `[bootstrap.packages]`" effect=write {
            alias up
            long_help #"""
...
"""#
            ...
            flag "-y --yes" help="Skip the confirmation prompt"
            arg <PACKAGE>… help="Packages in `manager:package[@version]` form" var=#true
        }
    }
    ...
    cmd remote help="Bootstrap one or more machines over OpenSSH" effect=destructive {
        ...
        flag --fail-fast help="Stop after the first failed target"
        flag --force-dotfiles help="Allow remote dotfile conflicts to be replaced"
        flag --host help="Ad-hoc SSH destination (`[user@]host`); repeat for multiple hosts" var=#true {
            arg "<[USER@]HOST>"
        }
        flag "-i --identity-file" help="SSH identity file override" {
            arg <IDENTITY_FILE>
        }
        ...
        flag --ssh-option help="OpenSSH `-o` option; repeat for multiple options" var=#true {
            arg <OPTION>
        }
        flag --tag help="Select configured hosts with this tag; repeat to match any tag" var=#true {
            arg <TAG>
        }
    ...
    cmd repos subcommand_required=#true help="Manage git repo checkouts from `[bootstrap.repos]`" effect=read {
        ...
        cmd update effect=write {
            flag "-n --dry-run" help="Print the commands that would run without running them"
            flag "-y --yes" help="Skip the confirmation prompt"
            arg "[PATH]…" help="Update only matching configured or expanded paths" required=#false var=#true
        }
    }
    cmd secrets subcommand_required=#true help="Inspect bootstrap secret inputs without revealing their values" effect=read {
        cmd status help="Show whether declared bootstrap secret inputs are available" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 if a declared secret input is unavailable"
        }
    }
    cmd services subcommand_required=#true help="Manage Linux system services from `[bootstrap.services]`" effect=read {
        ...
        cmd status help="Show configured Linux system service state" effect=read {
            flag "-J --json" help="Output in JSON format"
            flag --missing help="Exit with code 1 when any service is not converged"
        }
    }
...
cmd cache help="Manage the mise cache" effect=read {
    long_help #"""
...
cmd completion help="Generate shell completions" effect=read {
    ...
    flag --include-bash-completion-lib help="Retained for compatibility with older completion generators." {
        long_help #"""
...
"""#
    ...
    flag --usage help="Retained for compatibility with older completion generators." hide=#true {
        long_help #"""
Retained for compatibility with older completion generators.
...
"""#
    ...
    flag --force help="Replace a file at the target path that mise did not write" requires=--install effect=write
...
"""#
    cmd activate hide=#true help="Output direnv function to use mise inside direnv" effect=read {
        long_help #"""
Output direnv function to use mise inside direnv
...
"""# effect=write
    cmd exec hide=#true help=#"""
[internal] This is an internal command that writes an envrc file
for direnv to consume.
"""#
}
cmd dotfiles hide=#true subcommand_required=#true help="Manage dotfiles from `[dotfiles]` (deprecated)" effect=read {
    long_help #"""
Manage dotfiles from `[dotfiles]` (deprecated)

Use `mise bootstrap dotfiles` instead.
"""#
    cmd add hide=#true help="Add or update dotfiles in `[dotfiles]`" effect=write {
        long_help #"""
Add or update dotfiles in `[dotfiles]`
...
"""#
    ...
    cmd apply hide=#true help="Apply dotfiles from `[dotfiles]`" effect=write {
        long_help #"""
Apply dotfiles from `[dotfiles]`
...
"""#
    ...
    cmd unapply hide=#true help="Remove dotfiles applied from `[dotfiles]`" effect=destructive {
        long_help #"""
Remove dotfiles applied from `[dotfiles]`
...
cmd generate subcommand_required=#true help="Generate files for various tools/services" effect=read {
    alias gen
    alias g hide=#true
    cmd bootstrap hide=#true help="Deprecated. Use `mise generate install-script` instead" effect=write {
        long_help #"""
...
"""#
    ...
    cmd install-script help="Generate a script to download+execute mise" effect=write {
        long_help #"""
Generate a script to download+execute mise
...
cmd github hide=#true subcommand_required=#true help="GitHub related commands" effect=read {
    cmd token hide=#true help="Display the GitHub token mise will use for a given host" effect=read {
        long_help #"""
...
cmd global hide=#true help="Sets/gets the global tool version(s)" effect=write {
    long_help #"""
...
cmd hook-env hide=#true help="[internal] called by activate hook to update env vars directory change" effect=read {
...
in future releases.
"""#
    cmd build help="[experimental] Build an OCI image from the current mise.toml" effect=write {
        long_help #"""
...
"""#
        ...
        flag --mount-point help="Where to place tool installs inside the image (default: /mise)" {
            arg <MOUNT_POINT>
        }
...
"""#
        ...
        flag "-e --env" help="Set environment variable in the container (repeatable, `KEY=VAL`)" var=#true {
            arg "<KEY=VAL>"
        }
        flag "-i --interactive" help="Run interactively (pass `-i` to the engine)"
        flag "-t --tty" help="Allocate a TTY (pass `-t` to the engine)"
        flag "-w --workdir" help="Working directory inside the container" {
            arg <WORKDIR>
        }
...
cmd patrons help="Show the individuals supporting mise as Patron-tier members" effect=read {
    long_help #"""
...
cmd deps help="[experimental] Manage project dependencies" effect=write {
    alias dep
    alias prepare hide=#true
    long_help #"""
...
"""#
    ...
    flag --only help="Run specific deps rule(s) only" var=#true {
        arg <ONLY>
    }
    flag --skip help="Skip specific deps rule(s)" var=#true {
        arg <SKIP>
    }
...
"""#
        flag "-D --dev" help="Add as a development dependency"
        arg <PACKAGES>… help="Package(s) to add (e.g., npm:react, npm:@types/react@19)" var=#true
    }
    cmd install help="Install all project dependencies" effect=write {
        long_help #"""
...
"""#
    ...
    cmd remove help="Remove a dependency" effect=destructive {
        long_help #"""
Remove a dependency
...
"""#
        arg <PACKAGES>… help="Package(s) to remove (e.g., npm:lodash)" var=#true
    }
}
cmd prune help="Delete unused versions of tools" effect=destructive {
    long_help #"""
...
cmd set help="Set environment variables in mise.toml" effect=write {
...
cmd sponsors help="Show the companies sponsoring mise and the jdx.dev open source tools" effect=read
...
"""# required=#false var=#true
    ...
    cmd edit help="Edit a task with $EDITOR" effect=write {
        long_help #"""
...
cmd usage hide=#true help="Generate a usage CLI spec" effect=read {
    long_help #"""
...
if not test -f "$spec_file"
    ...
    echo $_usage_spec_mise_2026_8_10 > "$spec_file"
end
