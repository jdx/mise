...
# current
complete -kxc mise -n "$fssf current" -d 'Forge to show versions of e.g.: ruby, node, cargo:eza, npm:prettier, etc'
...
complete -kxc mise -n "$fssf ls" -d 'Only show tool versions from [FORGE]'
...
complete -kxc mise -n "$fssf prune" -d 'Prune only versions from this forge(s)'
...
complete -kxc mise -n "$fssf use" -l remove -d 'Remove the forge(s) from config file'
